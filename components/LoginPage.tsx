...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (email && password) {
      ...
      alert(t('loginPage.alertMissingCredentials'));
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-background to-accent flex items-center justify-center p-4 font-sans">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 space-y-8 transform transition-all hover:scale-105 duration-300">
        <div className="text-center">
          ...
          <h1 className="mt-4 text-3xl font-extrabold text-text-primary">{t('loginPage.title')}</h1>
          <p className="mt-2 text-text-secondary">{t('loginPage.subtitle')}</p>
        </div>
        <form className="space-y-6" onSubmit={handleLoginSubmit}>
          <div>
            <label htmlFor="email" className="block text-sm font-medium text-text-secondary">{t('loginPage.emailLabel')}</label>
            <input
              type="email"
              id="email"
              value={email}
          ...
          <div>
            <label htmlFor="password" className="block text-sm font-medium text-text-secondary">{t('loginPage.passwordLabel')}</label>
            <input
              type="password"
              id="password"
              value={password}
        ...
        <div className="relative flex py-2 items-center">
          ...
          <span className="flex-shrink mx-4 text-gray-400">{t('loginPage.or')}</span>
          <div className="flex-grow border-t border-gray-300"></div>
        </div>
